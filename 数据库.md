...
> 分为垂直和水平,垂直是按列切分,水平是按行切分.
>
> 垂直把宽表的不同字段拆到不同的表或者数据库中,减少单行数据量.
>
> 水平把数据按照规则分散到多张表或者多个库.减少单表数据量
>
> 分表是在同一个数据库拆分成多个表,分库是拆分到不同的实例上,不仅解决数据量问题,还解决单数据库瓶颈问题.
>
> 水平分表指的是一张表拆分成多个小表,比如order_1,order_2
...
> 可用uuid,随机生成128位字符串作为id,但是作为主键对索引不好,会导致页断裂.
>
> 可用雪花算法,最主流,使用一个 64 位的数字来作为全局唯一 ID。符号位+时间戳+工作机器ID+12 位的序列号.

# 数据表

数据库表设计,用到的范式理论?

> 第一范式原子性，确保表的每一列都是不可分割的基本数据单元,后面的两个范式讲的是表的普通字段与主键的关系
>
...
> SELECT SUBSTRING('HelloWorld', 1, 5); -- Hello（从第1位开始取5个字符）
> SELECT SUBSTRING('HelloWorld', 6);-- World（从第6位取到末尾）
...
> 先找到那些慢sql,可以开启慢查询日志, 编辑mysql配置文件my.cnf,设置 slow_query_log 参数为 1. 通过`SHOW VARIABLES LIKE 'slow_query_log%';` 找到慢查询日志.
>
...
>
> 1. 加索引
>    1. 对select和where中涉及到的字段建立联合索引,实现覆盖索引
> 2. sql优化,用索引
> 3. 表结构优化
> 4. 架构优化
...
>
> from的join,where中字段索引,select的列,order by优化, limit/offset分页优化.
...
>
> - 当使用 inner join 时，MySQL 会选择数据量比较小的表作为驱动表，大表作为被驱动表。left join时,左表为驱动表. 让小表做驱动表.
>
...
对于邮箱,选取区分度较高的前几个字符建立前缀索引,前缀索引无法用到order by和group by,和覆盖索引的理由:

> 前缀索引是按照前几个字符建立的索引,没有保存全量字符串,order by和group by都需要全量字符串相比较,所以无法用到.

为什么要优化select *

> 为了利用覆盖索引,避免覆盖索引失效.为了避免返回冗余字段,导致网络开销和缓存空间增大.
...
limit offset,如何进行分页优化？

> 避免深度偏移带来的全表扫描,如 LIMIT 1000,20. 分页会导致变慢,由于 OFFSET 的存在，OFFSET 会导致 MySQL 必须扫描和跳过 offset + limit 条数据，这个过程是非常耗时的。
>
> 用延迟关联和添加书签.
>
> **延迟关联**的思路是：先用子查询只查主键，再用主键去关联原表取数据。
>
> 书签法:记住上一页最后一条记录的排序值，下一页直接从这个位置往后取，彻底跳过 `OFFSET`
>
> 延迟关联通用,支持跳转到第几页,书签法性能好但是只支持上一页和下一页.
>
> ```sql
...
>
> key如果为 PRIMARY,表示主键索引,为null表示没有是用索引
>
> type={system/const(只找到一行),range(索引范围扫描),eq_ref(对主键join匹配,确保join有索引),ref指的是非唯一索引匹配(如 `WHERE name = '王二'`，name 有普通索引),index(全索引扫描,如果需要回表则需要优化),ALL}如果 type=ALL 表示全表扫描
>
> extra取值三个: index(覆盖索引),temporary table,filesort 如果temporary table超过了内存Memory引擎大小则会导致sql显著变慢. Extra=Using filesort表示排序的字段不是索引,则可能需要优化.
>
> temporary table在group by或者distinct常用
...
当无法使用索引排序时，MySQL 需要在内存或磁盘中进行排序操作,就是filesort,filesort不一定用磁盘,数据量小时排序在内存的sort_buffer进行.

filesort具体怎么执行?

> 分为全字段排序和 rowid 排序两种算法。
>
> 全字段排序,首先读取所有要排序的行内容,也就是全字段.直接排序返回结果,不需要回表. 缺点是内存占用大,如果数量超过 sort buffer还需要借助磁盘.
>
> rowid中,比如主键是id,用age排序,不拿全部字段,而只拿id和age,在sort buffer排序,完毕后根据id进行回表,拿到全字段. 优点内存占用低,缺点需要两次IO
